...
export const getAllLeavingRequestsSchema = Joi.object({
  ...
  limit: Joi.number().integer().min(1).max(100).default(10),
  status: Joi.string().valid('PENDING', 'APPROVED', 'REJECTED', 'COMPLETED').optional(),
  sortBy: Joi.string().valid('createdAt', 'requestedLeaveDate', 'approvedAt').default('createdAt'),
...
export const approveOrRejectRequestSchema = Joi.object({
  ...
  pendingDues: Joi.number()
    ...
    .messages({
      'number.min': 'Pending dues cannot be negative'
    }),
